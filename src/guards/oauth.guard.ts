...
/**
 * [HLD] Authentication & Authorization
 * [STATUS: IMPLEMENTED OAuth foundation; client JWT and ops-role policy are CONCEPTUAL]
 * ------------------------------------------------------------------
 * Responsibility: establish separate client and operations trust domains on
 * one MCP server. The intended client domain verifies a Firebase JWT and
 * scopes access to `case.clientId == token.uid`. The intended ops domain uses
 * OAuth 2.1 + PKCE with an `ops`/`ops_admin` role claim to protect approval,
 * assignment, and submission actions.
 *
 * Why two domains: approval actions must be attributable to a specific human,
 * while a client must never be able to invoke a consequential ops tool.
 * This guard currently validates OAuth tokens and populates request identity;
 * role claims, Firebase verification, and denied-action audit writes belong
 * to the planned authorization/audit layer.
 * [/HLD]
 */
/**
 * OAuth Guard
...
export class OAuthGuard implements Guard {
  /**
   * [LLD] opsRoleGuard(ctx, requiredRole) — target behavior
   * - Run before every gated tool body; validate OAuth token and require a
   *   sufficient role claim. On success attach `{ opsUserId, role }` for audit.
   * - approve_documents/assign_broker require `ops`; irreversible submission
   *   additionally requires an explicit reason/confirmation.
   * - Reject missing/insufficient roles with 403 and emit `denied_action`;
   *   never fall back to allowing a valid token with no role.
   *
   * Current behavior: this method is the shared OAuth validation foundation.
   * Fine-grained role enforcement and audit emission are intentionally not yet
   * implemented, so no approval tools are exposed by this repository.
   */
